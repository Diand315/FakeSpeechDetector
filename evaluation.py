...
def calculate_tDCF_EER(cm_scores_file: str,
    ...
    if printout:
        spoof_cm_breakdown = {attack_type: cm_scores[cm_sources == attack_type] for attack_type in attack_types}
        eer_cm_breakdown = {}
...
def obtain_asv_error_rates(tar_asv, non_asv, spoof_asv, asv_threshold):
    ...
    Pmiss_spoof_asv = np.sum(spoof_asv < asv_threshold) / spoof_asv.size if spoof_asv.size != 0 else None
...
def compute_tDCF(bonafide_score_cm, spoof_score_cm, Pfa_asv, Pmiss_asv,
    ...
    if cost_model['Ptar'] < 0 or cost_model['Pnon'] < 0 or cost_model['Pspoof'] < 0 or \
       np.abs(cost_model['Ptar'] + cost_model['Pnon'] + cost_model['Pspoof'] - 1) > 1e-10:
        sys.exit('ERROR: Your prior probabilities must sum to one and be positive.')
    if Pmiss_spoof_asv is None:
        sys.exit('ERROR: Missing spoof miss rate for ASV system.')
    combined_scores = np.concatenate((bonafide_score_cm, spoof_score_cm))
    if np.isnan(combined_scores).any() or np.isinf(combined_scores).any():
        sys.exit('ERROR: Your scores contain nan or inf.')
    n_uniq = np.unique(combined_scores).size
    if n_uniq < 3:
        sys.exit('ERROR: Provide soft CM scores (not binary) for t-DCF.')
    Pmiss_cm, Pfa_cm, CM_thresholds = compute_det_curve(bonafide_score_cm, spoof_score_cm)
    C1 = cost_model['Ptar'] * (cost_model['Cmiss_cm'] - cost_model['Cmiss_asv'] * Pmiss_asv) - \
         cost_model['Pnon'] * cost_model['Cfa_asv'] * Pfa_asv
    C2 = cost_model['Cfa_cm'] * cost_model['Pspoof'] * (1 - Pmiss_spoof_asv)
    if C1 < 0 or C2 < 0:
        sys.exit('ERROR: Negative weights detected in t-DCF computation.')
    tDCF = C1 * Pmiss_cm + C2 * Pfa_cm
    tDCF_norm = tDCF / np.minimum(C1, C2)
    if print_cost:
        print('t-DCF evaluation from [Nbona={}, Nspoof={}] trials'.format(bonafide_score_cm.size, spoof_score_cm.size))
        print('t-DCF MODEL')
        print('   Ptar         = {:8.5f}'.format(cost_model['Ptar']))
        print('   Pnon         = {:8.5f}'.format(cost_model['Pnon']))
        print('   Pspoof       = {:8.5f}'.format(cost_model['Pspoof']))
        print('   Cfa_asv      = {:8.5f}'.format(cost_model['Cfa_asv']))
        print('   Cmiss_asv    = {:8.5f}'.format(cost_model['Cmiss_asv']))
        print('   Cfa_cm       = {:8.5f}'.format(cost_model['Cfa_cm']))
        print('   Cmiss_cm     = {:8.5f}'.format(cost_model['Cmiss_cm']))
        if C2 == np.minimum(C1, C2):
            print('   tDCF_norm(s) = {:8.5f} x Pmiss_cm(s) + Pfa_cm(s)'.format(C1 / C2))
        else:
            print('   tDCF_norm(s) = Pmiss_cm(s) + {:8.5f} x Pfa_cm(s)'.format(C2 / C1))
