...
def genSpoof_list(dir_meta: str, is_train: bool = False, is_eval: bool = False):
    """
    Read the protocol archive and generate a sample key-value list and (in training mode) a label dictionary
    """
    meta_dict = {}
    file_list = []
...
def pad(x: np.ndarray, max_len: int = 64600):
    """
    Pad or truncate the input array to a fixed length
    """
    x_len = x.shape[0]
    if x_len >= max_len:
        return x[:max_len]
...
def pad_random(x: np.ndarray, max_len: int = 64600):
    """
    Randomly extract a fixed length from the input array; if it is too short, repeat and fill it
    """
    x_len = x.shape[0]
    if x_len >= max_len:
...
def add_random_noise(audio: np.ndarray, noise_factor: float = 0.003):
    """
    Random noise enhancement, adjustable noise parameters
    """
...
class Dataset_ASVspoof2019_train(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(self.labels.get(key, None), dict):
